...
def trusted_reviewer_decisions(
    ...
    contract = _reviewed_contract(root, snapshot, finding)
...
def _reviewed_contract(
    ...
    renamed_destination = next(
        (
            destination
            for destination, source in snapshot.renamed_files.items()
            if source == finding.path
        ),
        "",
    )
    snapshot_path = renamed_destination or finding.path
    before, after = file_versions(root, snapshot, snapshot_path)
    selected = (
        before
        if finding.path in snapshot.deleted_files or renamed_destination
        else after
    )
    source = selected.decode("utf-8")
