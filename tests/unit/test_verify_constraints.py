...
def test_release_docs_consistency_blocks_when_current_identity_is_missing(
    ...
    assert any("README.md" in item and "1.0.0" in item for item in blockers)
...
def test_beginner_guide_accepts_current_1_0_0_path(tmp_path: Path) -> None:
    mem = tmp_path / ".ai-sdlc" / "memory"
    mem.mkdir(parents=True)
    (mem / "constitution.md").write_text("# C\n", encoding="utf-8")
    _copy_release_contract_surfaces(tmp_path)
...
def test_073_frontend_solution_confirmation_verification_surfaces_stale_public_primevue_visual_contract_as_blocker(
    tmp_path: Path,
) -> None:
    _write_073_checkpoint(tmp_path)
    _write_073_solution_confirmation_artifacts(tmp_path)
    ...
    payload = yaml.safe_load(provider_manifest_path.read_text(encoding="utf-8"))
    assert isinstance(payload, dict)
    payload["visual_contract_version"] = "v1.7.1"
    ...
    assert report.coverage_gaps == ("frontend_solution_confirmation_consistency",)
    assert any(
        "public-primevue visual_contract_version must be v1.8" in blocker
        for blocker in report.blockers
    )
