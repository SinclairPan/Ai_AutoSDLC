...
def test_calibration_candidate_contains_a_readable_probe_file(tmp_path: Path) -> None:
    ...
    sentinel_roots = (
        Path(context.candidate_root),
        *(Path(value) for value in context.peer_output_roots),
        Path(context.protected_home_root),
        Path(context.normalized_run_root).parent / "outside",
    )
    assert all(
        (root / DIRECTORY_SENTINEL_NAME).read_text(encoding="utf-8")
        == "calibration-readable"
        for root in sentinel_roots
    )
    protected_home = Path(context.protected_home_root)
    protected_configs = tuple(Path(value) for value in context.protected_config_roots)
    assert any(value.is_relative_to(protected_home) for value in protected_configs)
    assert any(
        not value.is_relative_to(protected_home) for value in protected_configs
    )
    runtime_read_roots = tuple(Path(value) for value in context.runtime_read_roots)
    assert any(
        value.is_relative_to(runtime_root)
        for value in protected_configs
        for runtime_root in runtime_read_roots
    )
    calibration_root = Path(context.normalized_run_root).parent
    local_runtime_roots = tuple(
        value
        for value in runtime_read_roots
        if value.is_relative_to(calibration_root)
    )
    assert local_runtime_roots
    assert all(
        (root / DIRECTORY_SENTINEL_NAME).read_text(encoding="utf-8")
        == "calibration-readable"
