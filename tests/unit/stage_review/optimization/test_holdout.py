...
def test_commitment_recovers_from_sealed_segment_and_keeps_sequence(
    ...
    first = store.commit(_request(1, payload_bytes=8192))
    second = store.commit(_request(2, payload_bytes=8192))
    ...
    ) as bundle, store.storage.acquire_planned_lease(
        prepared.lease_plan,
    ...
    assert store.commit(_request(1, payload_bytes=8192)) == first
