...
def test_late_critical_event_persists_unclassifiable_attribution(
    ...
    evidence_paths = tuple((store.root / "evidence").glob("*.json"))
    assert tuple(path.name for path in evidence_paths) == (
        f"{store.evidences()[0].attribution_input_digest.removeprefix('sha256:')}.json",
    )
