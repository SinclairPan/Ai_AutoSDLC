...
def test_trusted_release_ignores_runtime_timestamp_rollover(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import ai_sdlc.core.stage_review.codex_trusted_releases as releases

    registry = _registry_attestations()
    _bind_registry_digest(monkeypatch, registry)
    expected = releases._published_codex_attestation("linux", "x64")
    assert expected is not None
    attestations = iter(
        (
            expected.model_copy(update={"created_at": "2026-07-25T08:29:59Z"}),
            expected.model_copy(update={"created_at": "2026-07-25T08:30:00Z"}),
        )
    )
    monkeypatch.setattr(
        releases,
        "_published_codex_attestation",
        lambda *_args, **_kwargs: next(attestations),
    )

    trusted = releases._trusted_published_codex_release(
        "linux",
        "x64",
        registry_attestations=registry,
    )
