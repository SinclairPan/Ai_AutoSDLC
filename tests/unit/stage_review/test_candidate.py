...
def _snapshot_with_raw_alias(alias: str) -> SourceSnapshot:
    snapshot = _source_snapshot()
    identity = _digest("9")
    return snapshot.model_copy(
        update={
            "changed_files": [*snapshot.changed_files, alias],
            "raw_change_identities": {
                **snapshot.raw_change_identities,
                alias: identity,
            },
        }
    )
...
def test_public_candidate_rejects_windows_tail_alias_of_review_artifact() -> None:
    alias = _REVIEW_PATH.replace(".ai-sdlc/", ".ai-sdlc./", 1)

    with pytest.raises(ValueError, match="non-canonical review artifact alias"):
        _build_from_snapshot(_snapshot_with_raw_alias(alias))
...
def test_public_candidate_rejects_parent_escape_reentry_review_alias() -> None:
    alias = f".. /repository/{_REVIEW_PATH}"
