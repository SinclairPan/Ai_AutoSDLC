...
from ai_sdlc.core.stage_review import artifacts as stage_review_artifacts
...
def test_committed_activation_source_survives_temporary_cleanup_failure(
    tmp_path: Path,
    ...
    relative = activation_evidence_ingestor._store_source(
        shared,
        source,
        digest,
        ".json",
    ...
    assert len(stage_review_artifacts._pending_temporary_cleanup()) == 1
    monkeypatch.setattr(
        stage_review_artifacts,
        "_unlink_with_retry",
        original_unlink,
    )
    stage_review_artifacts._retry_pending_temporary_cleanup()
    assert stage_review_artifacts._pending_temporary_cleanup() == ()
