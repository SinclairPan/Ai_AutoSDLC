...
def test_reader_release_retries_windows_sharing_violation_during_writer_scan(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    project_id = resolve_repository_project_id(tmp_path)
    reader_entered = Event()
    writer_scanned = Event()
    ...
    def observed_read(path: Path) -> dict[str, object]:
    ...
    def transient_unlink(path: Path, *args: object, **kwargs: object) -> None:
        ...
        if (
            path.parent.name == "readers"
            and writer_scanned.is_set()
            and unlink_attempts == 0
        ):
            unlink_attempts += 1
            raise PermissionError(13, "simulated Windows sharing violation")
        original_unlink(path, *args, **kwargs)

    monkeypatch.setattr(activation_fence, "read_json_object", observed_read)
    ...
    writer = Thread(target=acquire_writer)
    reader.start()
    assert reader_entered.wait(5)
    writer.start()
    assert writer_scanned.wait(5)
    release_reader.set()
...
def test_hold_does_not_treat_repeated_reviews_of_one_candidate_as_independent(
    tmp_path: Path,
) -> None:
    project_id = resolve_repository_project_id(tmp_path)
    started = datetime(2026, 7, 1, tzinfo=UTC)
    ...
    for index in (1, 2):
        record_activation_safety_recovery(
            ...
            _recovery_sample(
                tmp_path,
                hold,
                f"session.same-candidate.{index}",
...
def _recovery_sample(
    ...
    *,
    ...
    completion = ReviewSessionCompletion(
        ...
        candidate_manifest_digest=_digest(f"candidate:{candidate_id or session_id}"),
