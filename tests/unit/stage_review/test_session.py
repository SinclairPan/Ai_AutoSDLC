...
def test_completed_operation_retries_transient_pointer_release(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    fixture, risk_profile = _unstarted(tmp_path)
    command = _start_command(fixture, risk_profile, suffix="pointer-release")
    original_unlink = Path.unlink
    failures = 0

    def transient_pointer_sharing_violation(
        path: Path,
        *args: object,
        **kwargs: object,
    ) -> None:
        nonlocal failures
        if path.name == "active-operation.json" and failures == 0:
            failures += 1
            raise PermissionError(32, "simulated Windows sharing violation")
        original_unlink(path, *args, **kwargs)

    monkeypatch.setattr(Path, "unlink", transient_pointer_sharing_violation)

    result = fixture.service.start(command)
