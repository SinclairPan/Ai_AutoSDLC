...
import ai_sdlc.core.stage_review.review_input_packet as review_input_packet_module
...
def test_review_packet_rejects_change_between_freshness_and_capture(
    tmp_path: Path,
    ...
    original_capture = review_input_packet_module.capture_path_changes

    def capture_after_mutation(root: Path, frozen: object) -> object:
        (source / "candidate.txt").write_text("raced", encoding="utf-8")
        return original_capture(root, frozen)  # type: ignore[arg-type]

    monkeypatch.setattr(
        review_input_packet_module,
