...
class _ExecutingBackend:
    ...
    def execute(self, command, permit):
        ...
        if self.result_override is not None:
            stdout = json.dumps(self.result_override, sort_keys=True)
            Path(command.argv[-1]).write_text(stdout, encoding="utf-8")
...
def test_remote_review_runs_only_after_dispatch_and_binds_both_receipts(
    ...
    assert rig.backend.last_command is not None
    assert rig.backend.last_command.argv[0] == str(Path(sys.executable).resolve())
    assert rig.backend.last_command.argv[1:3] == ("-I", "-c")
    assert "def _validate_response" in rig.backend.last_command.argv[3]
    assert json.loads(rig.backend.last_command.stdin_text) == _valid_response()
    assert rig.backend.last_command.stdin_text.isascii()
    assert not tuple(rig.output_root.glob("*.provider-response.json"))
    assert len(tuple(rig.output_root.glob("*.review-result.json"))) == 1
...
def test_remote_review_rejects_schema_valid_result_not_bound_to_egress_response(
    tmp_path: Path,
) -> None:
    isolated_result = _valid_response()
    isolated_result["provider_call_id"] = "provider-call.result-substitution"
    rig = _review_rig(
        tmp_path,
        _valid_response(),
        isolated_result_override=isolated_result,
    )
    rig.journal.prepare(rig.request, lease_owner=_OWNER, now=_now())
...
def _review_rig(
    ...
    *,
    remote: bool = True,
    isolated_result_override: dict[str, object] | None = None,
) -> _ReviewRig:
    ...
    backend = _ExecutingBackend(
        ...
        result_override=isolated_result_override,
    )
    ...
    driver = RemoteReviewDriver(
        ...
        validation_python_executable=str(Path(sys.executable).resolve()),
    )
...
def _valid_response() -> dict[str, object]:
    return {
        "provider_call_id": "provider-call.远端评审",
        "review": {
            "verdict": "passed",
            "coverage": {"reviewed_area_ids": ["capability.security"]},
            "findings": [],
