...
class ContextValidatingBackend(FakeTrustedBackend):
    def probe(self, context, now):
        from ai_sdlc.core.stage_review.codex_isolation_policy import (
            _context_paths_valid,
        )
        from ai_sdlc.core.stage_review.isolation_execution import (
            build_isolation_evidence_manifest,
        )

        manifest = super().probe(context, now)
        if _context_paths_valid(context):
            return manifest
        payload = manifest.model_dump(
            mode="json",
            exclude={"artifact_kind", "manifest_digest"},
        )
        payload["boundary_results"] = ()
        return build_isolation_evidence_manifest(**payload)
...
def test_codex_sandbox_uses_short_read_only_invocation_spec(
    ...
    assert captured["profile"].index(
        f'{json.dumps(context.normalized_run_root)} = "write"'
    ) < captured["profile"].index(f'{json.dumps(str(bootstrap_root))} = "read"')
...
def test_codex_profile_preparation_rolls_back_private_and_bootstrap_roots(
    ...
    bootstrap_root = Path(context.normalized_run_root) / f".ai-sdlc-bootstrap-{nonce}"
...
def test_boundary_probe_moves_program_and_payload_off_command_line(
    ...
    execution, launcher_api = _apis()
    _, _, request = _journal_setup(tmp_path)
    ordinary = _context(launcher_api, request, _host(execution), tmp_path)
    context = policy._calibration_context(ordinary)
    policy._seed_calibration_targets(context)
    _seed_context_canaries(context)
    config_root, disposable_home = boundary.write_profile(context)
    captured: dict[str, object] = {}
    ...
    def _capture(
        ...
        *,
        ...
        del executable, supplied, config, home
        captured["argv"] = argv
        captured["payload"] = json.loads(stdin_text)
        captured["launch_envelope"] = launch_envelope
    ...
    assert captured["payload"]["candidate_target"]["path"] == context.candidate_root
    assert (
        captured["payload"]["launch_contract"] == captured["launch_envelope"].payload()
    )
...
def test_boundary_probe_payload_uses_host_verified_calibration_targets(
    ...
    policy._seed_calibration_targets(context)
    boundary.write_profile(context)
    sentinel_path = Path(context.candidate_root) / DIRECTORY_SENTINEL_NAME

    with sentinel_path.open("rb") as sentinel:
        payload = boundary._probe_payload(context, [], sentinel.fileno())
        assert boundary._probe_targets_unchanged(
            payload,
            sentinel.fileno(),
        )
        tampered = json.loads(json.dumps(payload))
        tampered["sentinel_fd"] = sentinel.fileno() + 10_000
        assert not boundary._probe_targets_unchanged(
            tampered,
            sentinel.fileno(),
        )
        runtime_read = Path(payload["runtime_read_target"]["path"])
        runtime_read.rename(runtime_read.with_name("runtime-read-original"))
        runtime_read.write_text("replaced", encoding="utf-8")
        assert not boundary._probe_targets_unchanged(
            payload,
            sentinel.fileno(),
        )
    ...
    calibration_root = Path(context.normalized_run_root).parent.resolve()
    assert all(item["exists"] is True for item in descriptors)
    assert payload["candidate_target"]["kind"] == "directory"
    assert payload["candidate_read_target"]["kind"] == "file"
    assert payload["global_config_targets"][0]["kind"] == "file"
    assert payload["boundary_link_target"]["kind"] == "directory"
    assert payload["boundary_link_read_target"]["kind"] == "file"
    assert payload["outside_target"]["visibility"] == "hidden"
    assert all(
        item["schema_version"] == "ai-sdlc-probe-target.v1" for item in descriptors
    )
    assert all(
        item["descriptor_digest"] == boundary._descriptor_digest(item)
        for item in descriptors
    )
    for item in descriptors:
        path = Path(item["path"])
        assert path.is_relative_to(calibration_root)
        assert path.resolve(strict=False).is_relative_to(calibration_root)


def test_boundary_probe_discards_all_evidence_when_host_target_identity_changes(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from ai_sdlc.core.stage_review import codex_isolation_boundary as boundary
    ...
    def swap_outside(*_args: object, **_kwargs: object) -> boundary.SandboxRun:
        outside = Path(context.normalized_run_root).parent / "outside"
        replaced = outside.with_name("outside-before-identity-swap")
        outside.rename(replaced)
        outside.mkdir()
        (outside / DIRECTORY_SENTINEL_NAME).write_text(
            "replacement",
            encoding="utf-8",
        )
        return boundary.SandboxRun(0, '{"boundary_results":[]}', "", 2718)

    monkeypatch.setattr(boundary, "run_sandbox", swap_outside)
...
def test_boundary_probe_payload_rejects_target_outside_calibration_root(
    tmp_path: Path,
) -> None:
    from ai_sdlc.core.stage_review import codex_isolation_boundary as boundary
    from ai_sdlc.core.stage_review import codex_isolation_policy as policy
    ...
    policy._seed_calibration_targets(context)
    boundary.write_profile(context)
    external = tmp_path / "external-protected-config"
    external.write_text("must-not-be-probed", encoding="utf-8")
    escaped = replace(context, protected_config_roots=(str(external),))
    sentinel_path = Path(context.candidate_root) / DIRECTORY_SENTINEL_NAME

    with (
        sentinel_path.open("rb") as sentinel,
        pytest.raises(ValueError, match="escapes canonical calibration root"),
    ):
        boundary._probe_payload(escaped, [], sentinel.fileno())

    assert external.read_text(encoding="utf-8") == "must-not-be-probed"
...
def test_codex_profile_omits_config_deny_covered_by_protected_home(
    tmp_path: Path,
) -> None:
    from ai_sdlc.core.stage_review.codex_isolation_backend import _profile_text
    ...
    assert f'{json.dumps(context.protected_home_root)} = "deny"' in profile
    assert f'{json.dumps(context.protected_config_roots[0])} = "deny"' not in profile


def test_codex_profile_preserves_config_deny_outside_protected_home(
    tmp_path: Path,
) -> None:
    from ai_sdlc.core.stage_review.codex_isolation_backend import _profile_text
    ...
    _, _, request = _journal_setup(tmp_path)
    context = _context(launcher_api, request, _host(execution), tmp_path)
    external_config = tmp_path / "protected-config" / ".gitconfig"
    context = replace(context, protected_config_roots=(str(external_config),))

    profile = _profile_text(context)

    assert f'{json.dumps(str(external_config))} = "deny"' in profile


def test_codex_profile_preserves_config_deny_reopened_by_runtime_read(
    tmp_path: Path,
) -> None:
    from ai_sdlc.core.stage_review.codex_isolation_backend import _profile_text
    ...
    _, _, request = _journal_setup(tmp_path)
    context = _context(launcher_api, request, _host(execution), tmp_path)
    runtime = Path(context.protected_home_root) / "trusted-runtime"
    protected_config = runtime / "secret.json"
    context = replace(
        context,
        protected_config_roots=(str(protected_config),),
...
def test_codex_profile_preserves_config_deny_reopened_by_trusted_read(
    tmp_path: Path,
) -> None:
    from ai_sdlc.core.stage_review.codex_isolation_backend import _profile_text
    ...
    _, _, request = _journal_setup(tmp_path)
    context = _context(launcher_api, request, _host(execution), tmp_path)
    trusted = Path(context.protected_home_root) / "trusted-bootstrap"
    protected_config = trusted / "secret.json"
    context = replace(context, protected_config_roots=(str(protected_config),))

    profile = _profile_text(context, trusted_read_paths=(trusted,))

    assert f'{json.dumps(str(protected_config))} = "deny"' in profile


def test_codex_profile_preserves_config_deny_reopened_by_candidate_read(
    tmp_path: Path,
) -> None:
    from ai_sdlc.core.stage_review.codex_isolation_backend import _profile_text
    ...
    _, _, request = _journal_setup(tmp_path)
    context = _context(launcher_api, request, _host(execution), tmp_path)
    candidate = Path(context.protected_home_root) / "candidate"
    protected_config = candidate / "secret.json"
    context = replace(
        context,
...
def test_codex_profile_rejects_read_root_equal_to_protected_config(
    tmp_path: Path,
) -> None:
    from ai_sdlc.core.stage_review.codex_isolation_backend import _profile_text
    ...
    _, _, request = _journal_setup(tmp_path)
    context = _context(launcher_api, request, _host(execution), tmp_path)
    protected_config = Path(context.protected_home_root) / "runtime"
    context = replace(
        context,
        protected_config_roots=(str(protected_config),),
        runtime_read_roots=(str(protected_config),),
    )

    with pytest.raises(ValueError, match="protected config and read root are equal"):
        _profile_text(context)


def test_codex_profile_preserves_config_deny_when_resolved_read_root_covers_it(
    tmp_path: Path,
) -> None:
    from ai_sdlc.core.stage_review.codex_isolation_backend import _profile_text
    ...
    _, _, request = _journal_setup(tmp_path)
    context = _context(launcher_api, request, _host(execution), tmp_path)
    protected_home = Path(context.protected_home_root)
    protected_home.mkdir(parents=True)
    runtime = protected_home / "trusted-runtime"
    runtime.mkdir()
    runtime_alias = tmp_path / "trusted-runtime-alias"
    runtime_alias.symlink_to(runtime, target_is_directory=True)
    protected_config = runtime / "secret.json"
    context = replace(
        context,
        protected_config_roots=(str(protected_config),),
        runtime_read_roots=(str(runtime_alias),),
    )
...
def test_codex_policy_rejects_equal_config_and_runtime_read_before_calibration(
    tmp_path: Path,
) -> None:
    from ai_sdlc.core.stage_review.codex_isolation_policy import (
        _context_paths_valid,
    ...
    _, _, request = _journal_setup(tmp_path)
    context = _context(launcher_api, request, _host(execution), tmp_path)
    protected_config = context.protected_config_roots[0]
    context = replace(context, runtime_read_roots=(protected_config,))

    assert _context_paths_valid(context) is False


def test_codex_policy_rejects_resolved_equal_config_and_runtime_read(
    tmp_path: Path,
) -> None:
    from ai_sdlc.core.stage_review.codex_isolation_policy import (
        _context_paths_valid,
    ...
    _, _, request = _journal_setup(tmp_path)
    context = _context(launcher_api, request, _host(execution), tmp_path)
    protected_config = Path(context.protected_config_roots[0])
    protected_config.parent.mkdir(parents=True, exist_ok=True)
    protected_config.write_text("secret", encoding="utf-8")
    alias = tmp_path / "protected-config-alias"
    alias.symlink_to(protected_config)
    context = replace(context, runtime_read_roots=(str(alias),))

    assert _context_paths_valid(context) is False


def test_equal_config_and_runtime_read_is_refused_before_permit(
    tmp_path: Path,
) -> None:
    execution, launcher_api = _apis()
    _, _, request = _journal_setup(tmp_path)
    host = _host(execution)
    backend = ContextValidatingBackend(
        _manifest(execution, host_snapshot_digest=host.snapshot_digest)
    )
    launcher = launcher_api.ReviewerIsolationLauncher(
        tmp_path,
    ...
    context = replace(
        context,
        runtime_read_roots=(context.protected_config_roots[0],),
    )

    wrapped = launcher.prepare_driver(driver, context=context, now=_now())
