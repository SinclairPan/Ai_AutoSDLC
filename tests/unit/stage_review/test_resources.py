...
def test_platform_lock_failures_fail_closed(
    ...
    monkeypatch.setattr(
        "ai_sdlc.core.stage_review.artifacts._try_acquire_os_lock", fail_lock
    )
    result = governor.reserve_admission(
        _envelope(),
        budget_policy=_policy(),
...
def test_atomic_artifact_temp_names_fit_windows_max_path(
    ...
    assert atomic_target.read_bytes() == b'{"kind":"atomic"}\n'
    assert exclusive_target.read_bytes() == b'{"kind":"exclusive"}\n'


def test_short_file_lock_uses_stable_file_and_releases_os_lock(
    tmp_path: Path,
) -> None:
    lock_path = tmp_path / "mutation.lock"

    with (
        artifacts.ShortFileLock(lock_path, timeout_seconds=1),
        pytest.raises(artifacts.ResourceLockUnavailableError),
    ):
        assert lock_path.is_file()
        with artifacts.ShortFileLock(lock_path, timeout_seconds=0.02):
            raise AssertionError("contender acquired the held OS lock")

    assert lock_path.is_file()
    with artifacts.ShortFileLock(lock_path, timeout_seconds=1):
        pass


def test_exclusive_create_uses_unique_temporary_names_when_clock_repeats(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    call_lock = Lock()
    call_index = 0
    original_open = artifacts._open_exclusive

    def synchronized_open(path: Path) -> int:
        nonlocal call_index
        with call_lock:
            current = call_index
            call_index += 1
    ...
    monkeypatch.setattr(artifacts.time, "monotonic_ns", lambda: 1)
    monkeypatch.setattr(artifacts, "_open_exclusive", synchronized_open)

    with ThreadPoolExecutor(max_workers=2) as pool:
        futures = tuple(
            pool.submit(create_json_exclusive, target, {"worker": index})
            for index in range(2)
        )
        results = tuple(future.result(timeout=10) for future in futures)

    assert sorted(results) == [False, True]
    assert read_json_object(target)["worker"] in {0, 1}


def test_exclusive_create_result_survives_owned_temporary_cleanup_failure(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    target = tmp_path / "committed.json"
    original_unlink = artifacts._unlink_with_retry
    ...
    assert create_json_exclusive(target, {"committed": True}) is True
    assert read_json_object(target) == {"committed": True}
    pending = tuple(artifacts._pending_temporary_cleanup())
    assert len(pending) == 1
    assert pending[0].is_file()

    monkeypatch.setattr(artifacts, "_unlink_with_retry", original_unlink)
    artifacts._retry_pending_temporary_cleanup()

    assert artifacts._pending_temporary_cleanup() == ()
    assert pending[0].exists() is False


def test_unlink_retry_preserves_missing_ok_after_terminal_race(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    target = tmp_path / "cleanup-race.json"
    target.write_text("pending", encoding="utf-8")
    attempts = 0

    def sharing_violation_then_missing(_path: Path) -> None:
        nonlocal attempts
        attempts += 1
        if attempts <= 4:
            raise PermissionError(32, "simulated Windows sharing violation")
        raise FileNotFoundError(target)

    monkeypatch.setattr(Path, "unlink", sharing_violation_then_missing)

    assert artifacts._unlink_with_retry(target, missing_ok=True) is False
    assert attempts == 5


def test_unlink_retry_still_reports_persistent_sharing_violation(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    target = tmp_path / "persistent-sharing-violation.json"
    target.write_text("pending", encoding="utf-8")
    attempts = 0

    def persistent_sharing_violation(_path: Path) -> None:
        nonlocal attempts
        attempts += 1
        raise PermissionError(32, "simulated persistent Windows sharing violation")

    monkeypatch.setattr(Path, "unlink", persistent_sharing_violation)

    with pytest.raises(PermissionError, match="persistent Windows"):
        artifacts._unlink_with_retry(target, missing_ok=True)
    assert attempts == 5


def test_windows_pid_probe_distinguishes_exited_process_with_open_handle(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    kernel32 = _FakeKernel32(exit_code=259, wait_result=0)
    monkeypatch.setattr(
        artifacts.ctypes,
        "WinDLL",
    ...
    assert artifacts._windows_pid_is_active(1234) is False
    assert kernel32.closed_handles == [kernel32.handle]
...
class _FakeKernel32:
    def __init__(self, *, exit_code: int, wait_result: int) -> None:
        self.handle = 9876
        self.exit_code = exit_code
        self.wait_result = wait_result
        self.closed_handles: list[int] = []
        self.OpenProcess = _FakeWindowsCall(self._open_process)
        self.GetExitCodeProcess = _FakeWindowsCall(self._get_exit_code_process)
        self.WaitForSingleObject = _FakeWindowsCall(self._wait_for_single_object)
        self.CloseHandle = _FakeWindowsCall(self._close_handle)

    def _open_process(self, *_args: object) -> int:
        return self.handle

    def _get_exit_code_process(self, _handle: object, target: object) -> bool:
        target._obj.value = self.exit_code  # type: ignore[attr-defined]
        return True

    def _wait_for_single_object(self, _handle: object, _milliseconds: int) -> int:
