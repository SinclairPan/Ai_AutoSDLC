...
def test_receipt_store_replays_canonically_identical_provider_response(
    tmp_path: Path,
) -> None:
    store = FilesystemReviewReceiptArtifactStore(
        tmp_path,
        project_id="project.receipt-replay",
    )
    response = {"coverage": ("capability.correctness",)}

    digest = store.persist_response(response)

    assert store.persist_response(response) == digest
    assert store.resolve_response(digest) == {"coverage": ["capability.correctness"]}
