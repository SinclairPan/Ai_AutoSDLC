...
def test_foreground_scope_cleanup_failure_never_replaces_command_result(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    class BrokenCleanupLease:
        def __init__(self, *_args: object, **_kwargs: object) -> None:
            pass

        def __enter__(self) -> BrokenCleanupLease:
            return self
    ...
    monkeypatch.setattr(
        "ai_sdlc.cli.optimization_hooks.ForegroundExecutionLease",
        BrokenCleanupLease,
    )
...
def test_foreground_scope_preserves_body_failure_when_cleanup_also_fails(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    class BrokenCleanupLease:
    ...
    with (
        pytest.raises(ValueError, match="foreground failed"),
        foreground_optimization_scope(tmp_path, dry_run=False),
    ):
