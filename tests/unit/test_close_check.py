"""Unit tests for close-check core logic."""
...
def test_close_check_default_skips_unlisted_docs_sc021(tmp_path: Path) -> None:
    """Unlisted docs/** files are scanned only when --all-docs is enabled."""
    root = tmp_path / "repo6"
    root.mkdir()
    _setup_repo(
        root,
...
def test_close_check_respects_synthetic_command_from_registry(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    """The command list is not frozen to two literals."""
    monkeypatch.setattr(
