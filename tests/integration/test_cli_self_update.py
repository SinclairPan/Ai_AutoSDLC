...
def _env(tmp_path, *, channel: str = "github-archive") -> dict[str, str]:
    return {
        ...
        "AI_SDLC_UPDATE_ADVISOR_TEST_LATEST_VERSION": "v1.0.1",
...
def test_self_update_evaluate_json_reports_actionable_github_archive(
    ...
    assert payload["upstream_latest_version"] == "1.0.1"
    assert payload["channel_latest_version"] == "1.0.1"
...
def test_self_update_check_auto_installs_unknown_channel_for_076_plus(
    ...
    env["AI_SDLC_UPDATE_ADVISOR_TEST_LATEST_VERSION"] = "v1.1.0"
...
def test_self_update_install_completes_without_manual_followup(monkeypatch, tmp_path) -> None:
    ...
    def fake_extract(archive_path, extract_root, hint):
        ...
        (wheels / "ai_sdlc-1.0.1-py3-none-any.whl").write_text(
    ...
    result = runner.invoke(
        app,
        ["self-update", "install", "--version", "1.0.1"],
        env=_env(tmp_path),
    )
    ...
    assert "ai-sdlc --version => 1.0.1" in result.output
...
def test_self_update_install_allows_module_entry_without_bare_cli(
    ...
    assert result.exit_code == 0
    assert "更新完成" in result.output
    assert "已校验安装版本：AI-SDLC 1.0.1" in result.output
...
def test_self_update_installs_wheel_matching_requested_version(
    ...
    wheels = bundle / "wheels"
    wheels.mkdir(parents=True)
    (wheels / "ai_sdlc-1.0.0-py3-none-any.whl").write_text("old", encoding="utf-8")
    target = wheels / "ai_sdlc-1.1.0-py3-none-any.whl"
    ...
    self_update_cmd._install_bundle_into_current_runtime(bundle, "1.1.0")
...
def test_self_update_bare_cli_validation_does_not_mask_posix_path_gaps(
    ...
    with pytest.raises(self_update_cmd.SelfUpdateError):
        self_update_cmd._verify_bare_cli_version("1.1.0")
...
def test_self_update_rejects_tar_link_members(tmp_path) -> None:
    ...
    with tarfile.open(archive_path, "w:gz") as archive:
        root = tarfile.TarInfo("ai-sdlc-offline-1.0.1-macos-arm64")
        root.type = tarfile.DIRTYPE
        archive.addfile(root)

        link = tarfile.TarInfo("ai-sdlc-offline-1.0.1-macos-arm64/link")
        link.type = tarfile.SYMTYPE
        link.linkname = "/tmp"
        archive.addfile(link)

        payload = b"payload"
        member = tarfile.TarInfo("ai-sdlc-offline-1.0.1-macos-arm64/link/payload")
    ...
    with pytest.raises(self_update_cmd.SelfUpdateError, match="link member"):
        self_update_cmd._extract_release_asset(
            ...
            {
                "archive": "tar.gz",
                "filename": "ai-sdlc-offline-1.0.1-macos-arm64.tar.gz",
            },
        )
...
def test_self_update_instructions_are_user_result_oriented(tmp_path) -> None:
    result = runner.invoke(
        app,
        ["self-update", "instructions", "--version", "1.0.1"],
        env=_env(tmp_path),
    )
