...
def test_shadow_candidate_never_omits_staged_changes(tmp_path: Path) -> None:
    ...
    assert preflight.candidate is not None
    assert preflight.source_snapshot is not None
    assert preflight.source_snapshot.source_kind == "local-worktree"
    assert preflight.source_snapshot.changed_files == ["README.md", "src/app.py"]
...
def test_shadow_candidate_protects_both_sides_of_rename(tmp_path: Path) -> None:
    _init_git_repo(tmp_path)
    _write(tmp_path, "src/old.py", "VALUE = 1\n")
    _git(tmp_path, "add", "src/old.py")
    _git(tmp_path, "commit", "-m", "add old source")
    _git(tmp_path, "mv", "src/old.py", "src/new.py")
    loop_run = LoopRun(
        loop_id="implementation.rename",
        loop_type=LoopType.IMPLEMENTATION,
        status=LoopStatus.PASSED,
        work_item_id="001-rename",
        current_round=1,
        rounds=[LoopRound(round_number=1, status=LoopStatus.PASSED)],
    )
    prepared = prepare_loop_stage_close(
        ...
        target_status="closed",
        close_artifact_path=(
            tmp_path / ".ai-sdlc/loops/implementation/rename/close.json"
        ),
    )
    ...
    assert preflight.candidate is not None
    assert preflight.source_snapshot is not None
    assert preflight.source_snapshot.renamed_files == {
        "src/new.py": "src/old.py"
    }
    assert preflight.candidate.protected_source_set == [
        "src/new.py",
        "src/old.py",
    ]
...
def test_active_safety_hold_runs_recovery_review_and_blocks_writer(
    ...
    assert writes == 0
    assert len(tuple((shared / "activation/safety-recovery").glob("*.json"))) == 1
    assert len(executor.session_ids) == 1
    ...
    assert len(set(executor.session_ids)) == 2
...
def test_untrusted_ambient_read_lease_cannot_bypass_final_refresh(
    ...
    class AmbientLeaseEnforcer:
        def enforce_close(
            ...
            with activation_safety_read_lease(tmp_path, project_id):
                return guarded_writer()
    ...
    with pytest.raises(
        StageCloseGateUnavailableError,
        match="activation-safety-read-lease-untrusted",
    ):
        StageCloseGateway(close_enforcer=AmbientLeaseEnforcer()).execute(
            prepared,
            writer,
        )
...
def test_phase_one_writer_read_lease_blocks_phase_two_promotion(
    ...
    def writer() -> dict[str, str]:
        writer_started.set()
        assert allow_writer.wait(_CONCURRENCY_TIMEOUT_SECONDS)
    ...
    close_thread.start()
    assert writer_started.wait(_CONCURRENCY_TIMEOUT_SECONDS)
    promote_thread = Thread(target=promote)
    promote_thread.start()
    assert promotion_committed.wait(0.2) is False
    allow_writer.set()
    close_thread.join(timeout=_CONCURRENCY_TIMEOUT_SECONDS)
    promote_thread.join(timeout=_CONCURRENCY_TIMEOUT_SECONDS)
...
def test_shadow_derived_state_keeps_read_lease_until_attestation_is_persisted(
    ...
    def persist_with_pause(root, attestation) -> None:
        derived_started.set()
        assert allow_derived.wait(_CONCURRENCY_TIMEOUT_SECONDS)
    ...
    close_thread = Thread(target=close_stage)
    close_thread.start()
    assert derived_started.wait(_CONCURRENCY_TIMEOUT_SECONDS)
    mutation_thread = Thread(target=mutate_activation_state)
    mutation_thread.start()
    assert mutation_committed.wait(0.2) is False
    allow_derived.set()
    close_thread.join(timeout=_CONCURRENCY_TIMEOUT_SECONDS)
    mutation_thread.join(timeout=_CONCURRENCY_TIMEOUT_SECONDS)
...
class _AuthoritativeCompletedReviewExecutor:
    ...
    def execute(
        ...
        self.session_ids.append(candidate.review_session_id)
        scope = FindingScope(
