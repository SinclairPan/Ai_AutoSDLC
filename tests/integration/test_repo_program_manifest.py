...
def test_public_release_excludes_internal_program_history() -> None:
    ...
    assert not any(
        path.is_file()
        for path in (repo_root / ".ai-sdlc" / "work-items").rglob("*")
    )


def test_public_release_contains_only_current_release_notes() -> None:
    repo_root = Path(__file__).resolve().parents[2]
    release_dir = repo_root / "docs" / "releases"

    assert sorted(path.name for path in release_dir.glob("*.md")) == ["v1.0.0.md"]
