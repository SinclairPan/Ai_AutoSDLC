...
from ai_sdlc.core.stage_review.remote_review_driver import (
    build_remote_review_validation_command,
)
...
def test_final_coordinator_path_executes_isolated_or_refuses_before_command(
    ...
    result_path = Path(layout.output_root) / "review-result.json"
    ...
    assert json.loads(result_path.read_text(encoding="utf-8")) == exchange.response
    assert not tuple(Path(layout.output_root).glob("*.provider-response.json"))
...
def test_equal_config_and_runtime_read_refuses_before_permit(
    tmp_path: Path,
) -> None:
    now = datetime.now(UTC)
    host = _actual_host_snapshot()
    ...
    driver = _DetectedOnlyForbiddenDriver()
    launcher_root = tmp_path / "equal-config-read-launcher"
    launcher = ReviewerIsolationLauncher(
        launcher_root,
        registry=TrustedIsolationBackendRegistry.default(),
        backend=_actual_backend(),
    ...
    assert launcher.receipts()[-1].reason_id in expected_reasons
    assert not tuple(launcher_root.rglob("isolation-execution/consumed/*.json"))
    assert _canary_payloads(canaries) == before
...
def _actual_backend():
    return _DiagnosticCodexPermissionProfileBackend(
        _actual_codex_path(),
        release_manifest=_measured_trusted_release(),
    )


class _DiagnosticCodexPermissionProfileBackend(CodexPermissionProfileBackend):
    """仅在受控 E2E 失败时公开子进程诊断，避免 CI 丢失平台根因。"""

    def execute_journaled(self, command, permit, recorder):
        result = super().execute_journaled(command, permit, recorder)
        if result.return_code != 0:
            print(
                "controlled E2E isolated command failed: "
                f"return_code={result.return_code}; "
                f"stdout_tail={result.stdout[-800:]!r}; "
                f"stderr_tail={result.stderr[-800:]!r}",
                flush=True,
            )
        return result
...
def _controlled_review_exchange(
    ...
    def endpoint(envelope: ProviderTransportEnvelope) -> dict[str, object]:
        ...
        if not candidate_matches:
            raise AssertionError("controlled reviewer candidate lineage diverged")
        return {
            "provider_call_id": "provider-call.受控远端评审",
            "review": {
                "verdict": "passed",
                "coverage": {"reviewed_area_ids": ["capability.security"]},
                "findings": [],
                "evidence_digests": ["sha256:e2e-review-evidence"],
            },
...
def _validate_e2e_submission(submission: ProviderSubmission) -> str:
    review = RemoteReviewOutput.model_validate(submission.output_payload)
    if (
...
# E2E 复用生产命令构造器，避免维护一套只在测试中成立的 reviewer 协议。
# 受控 transport 只提供已验证响应，隔离校验仍在真实 OS sandbox 内执行。
class _E2ECommandDriver:
    ...
    def decode_isolated_result(self, request, command_kind, result):
        ...
        if payload != persisted or persisted != self._response:
