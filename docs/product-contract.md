...
- 010 的第二项外部 GO 前置条件是 `active no-bypass tag ruleset protects software and Certificate tags`：远端 active ruleset 必须精确覆盖软件 tag 与 generation-0 Certificate tag，允许新建但拒绝更新、删除和非快进变更，且不存在 bypass。009 中环境与 tag-ruleset 两个验证开关均保持字符串 `false`；部分 namespace、环境或 ruleset 失败均执行 terminal generation burn，不清理、不恢复、不重跑。
