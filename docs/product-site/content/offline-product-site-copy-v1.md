...
需求不是一段聊天记录，完成也不是一句“代码已经写好”。在正式绑定 WorkItem 的交付路径中，AI-SDLC 将目标、设计、任务、执行、验证和 Close 组织为可追踪的项目工件链。

#### 证据先于完成

测试、构建、浏览器结果和审查结论必须属于当前输入与当前候选。AI-SDLC 使用状态、工件和 digest 约束关闭条件；旧证据、缺失证据或漂移证据不能被一句总结替代。

#### 有界专家复核

复核不是让更多 Agent 无限讨论。AI-SDLC 按结果风险选择最多两名临时只读专家，要求它们读取同一份冻结输入，只返回可定位 Findings；原 Writer 负责修复，并且最多进行一次复审。
