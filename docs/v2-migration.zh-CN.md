...
3. Requirement、Design Contract、Implementation、Frontend Evidence、Local PR Review 五类结果由 CLI 按内容选择最多两种专家角色，宿主 Agent 自动以独立只读上下文执行并调用 `loop review-record`；有发现时最多修复并复审一次，不要求用户手动触发专家。
