...
def build_review_input(
    ...
    *,
    ...
    all_paths = [path for path, _ in (*artifacts, *upstream)]
    ...
    digest_payload = {
        ...
        "artifacts": [_digest_record(path, content) for path, content in artifacts],
        "upstream": [_digest_record(path, content) for path, content in upstream],
    ...
    return ReviewInput(
        ...
        artifact_paths=[path for path, _ in artifacts],
        upstream_context_paths=[path for path, _ in upstream],
        risk_signals=normalized_signals,
    )
