...
def uncovered_activation_safety_holds_for_candidate(
    root: Path,
    holds: tuple[ActivationSafetyHold, ...],
    candidate: CandidateManifest,
) -> tuple[ActivationSafetyHold, ...]:
    """同一候选重试不得重复贡献独立的冻结恢复样本。"""

    project_id = resolve_repository_project_id(root)
    candidate_digest = candidate_binding_digest(candidate)
    covered = {
        (sample.hold_id, sample.hold_digest)
        for sample in _read_recovery_samples(root, project_id)
        if sample.candidate_manifest_digest == candidate_digest
    }
    return tuple(
        hold
        for hold in holds
        if (hold.hold_id, hold.hold_digest) not in covered
    )
...
def _independent_recovery_samples(
    ...
    selected: dict[str, ActivationSafetyRecoverySample] = {}
    for item in sorted(eligible, key=lambda value: value.sample_digest):
        selected.setdefault(item.candidate_manifest_digest, item)
