...
class FilesystemReviewReceiptArtifactStore:
    ...
    def persist_isolation_receipt(self, value: IsolationExecutionReceipt) -> None:
        trusted = IsolationExecutionReceipt.model_validate(
    ...
    def _persist(
        ...
        if canonical_payload(read_json_object(path), policy) != canonical_payload(
            payload,
            policy,
        ):
    ...
    def _read_payload(self, kind: str, identity: str) -> dict[str, object]:
        try:
            ...
            raise ReceiptArtifactError(
                "review execution artifact is unavailable"
            ) from exc
