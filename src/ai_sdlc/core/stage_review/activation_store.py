...
from ai_sdlc.core.stage_review.certificate_input_guard import read_certificate
...
def _recover_enforced_activation_session(
    root: Path,
    *,
    candidate: CandidateManifest,
    ...
    """从已提交评审与 close receipt 自动补写 crash window 中缺失的激活记录。"""
    ...
    shared = resolve_canonical_shared_state(root, trusted_candidate.project_id)
    session_root = session_scope_root(
        shared / "stage-review-sessions",
        trusted_candidate.project_id,
        scope,
    )
    completion_path = session_root / "completion.json"
    completion = ReviewSessionCompletion.model_validate(
        read_json_object(completion_path)
    )
    claim = authorization.claim
    certificate = read_certificate(
        session_root / "certificates" / f"{claim.certificate_id}.json"
    )
    candidate_digest = candidate_binding_digest(trusted_candidate)
    if (
        completion.scope != scope
        or completion.candidate_manifest_digest != candidate_digest
        or certificate.certificate_id != claim.certificate_id
        or certificate.certificate_digest != claim.certificate_digest
        or certificate.certificate_revision != claim.certificate_revision
        or certificate.scope != scope
        or certificate.candidate_manifest_digest != candidate_digest
        or certificate.panel_plan_digest != completion.panel_plan_digest
        or certificate.session_digest != completion.session_digest
        or certificate.finding_ledger_digest != completion.finding_ledger_digest
        or certificate.task_risk_profile_digest != risk_profile.profile_digest
    ):
        raise ValueError("enforce activation recovery lineage diverged")
    return _record_enforced_activation_session(
        root,
