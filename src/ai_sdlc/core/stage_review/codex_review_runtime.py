...
from ai_sdlc.core.stage_review.activation_fence import (
    _activation_safety_final_read_lease as activation_safety_final_read_lease,
)
...
from ai_sdlc.core.stage_review.activation_store import (
    _recover_enforced_activation_session as recover_enforced_activation_session,
)
...
def _execute_enforced_close(
    ...
    with activation_safety_final_read_lease(
        ...
        if policy.compatibility_mode != "strict":
            raise StageCloseGateUnavailableError(
                "activation-policy-read-only-before-product-commit"
            )
...
def _review_drivers(
    ...
    return RemoteReviewDriverFactory(
        ...
        validation_python_executable=sandbox_python_executable(),
    )
