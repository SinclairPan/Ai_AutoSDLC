...
CI_CERTIFICATE_BUNDLE_PATH = ".ai-sdlc/attestations/ci-certificate-bundle.json"
...
def read_ci_certificate_bundle(path: Path) -> CiStageCloseCertificateBundle:
    try:
        ...
        raise CiCertificateVerificationError(
            "CI certificate bundle is invalid"
        ) from exc
...
def _verify_post_review_changes(
    ...
    if any(
        path != CI_CERTIFICATE_BUNDLE_PATH
        and not review_artifact_path_allowed(path, exclusion)
        for path in paths
    ):
