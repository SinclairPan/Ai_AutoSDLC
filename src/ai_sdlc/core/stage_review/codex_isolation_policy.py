...
def _context_paths_valid(context: IsolationLaunchContext) -> bool:
    ...
    if not bool(required) or not all(Path(value).is_absolute() for value in paths):
        return False
    readable = (context.candidate_root, *context.runtime_read_roots)
    return not any(
        _paths_equal(protected, read_root)
        for protected in context.protected_config_roots
        for read_root in readable
    )
...
def _paths_equal(left: str, right: str) -> bool:
    ...
    resolved_left = Path(left).resolve(strict=False)
    resolved_right = Path(right).resolve(strict=False)
    return lexical_left == lexical_right or resolved_left == resolved_right
...
def _calibration_context(context: IsolationLaunchContext) -> IsolationLaunchContext:
    ...
    return replace(
        ...
        protected_config_roots=(
            str(root / "protected-home" / ".gitconfig"),
            str(root / "protected-config" / "external.gitconfig"),
            str(root / "trusted-runtime" / "protected.json"),
        ),
        runtime_read_roots=tuple(
            sorted({*context.runtime_read_roots, str(root / "trusted-runtime")})
        ),
    )
...
def _seed_calibration_targets(context: IsolationLaunchContext) -> None:
    directories = (
        ...
        *context.peer_output_roots,
        context.protected_home_root,
        str(Path(context.normalized_run_root).parent / "trusted-runtime"),
        str(Path(context.normalized_run_root).parent / "outside"),
    )
    for value in directories:
        target = Path(value)
        target.mkdir(parents=True, exist_ok=True)
        (target / DIRECTORY_SENTINEL_NAME).write_text(
