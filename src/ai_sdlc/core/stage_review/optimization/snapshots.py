...
class SnapshotControlService(SnapshotServiceSupportMixin):
    ...
    def request_revocation(
        ...
        *,
        ...
        try:
            return self.retry.run(
                operation_id,
                lambda _: self._request_revocation_once(
                    snapshot_digest,
                    reason=reason,
                    operation_id=operation_id,
                ),
    ...
    def _request_revocation_once(
        ...
        *,
        ...
        with self._storage_bundle("critical_recovery", operation_id) as bundle:
    ...
    def _bind_session_attempt(
        ...
        with self._storage_bundle("session_binding", refreshed.operation_id) as bundle:
            trusted = self.store.persist_binding_operation(
                refreshed,
                resource_bundle=bundle,
            )
