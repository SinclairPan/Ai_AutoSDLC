...
def _compact_storage_stream(runtime: OptimizationRuntime, stream_kind: str) -> bool:
    ...
    stale = storage.committed_loose_paths(stream_kind)
    if stale:
        operation_id = f"optimization-compaction-recovery.{stream_kind}"
        expected_head = storage.manifest().manifest_digest
        with storage.acquire_lease(
            owner_id=operation_id,
            scope="compaction_recovery",
            expected_head=expected_head,
        ) as lease:
            storage.recover_committed_loose(stream_kind, lease=lease)
